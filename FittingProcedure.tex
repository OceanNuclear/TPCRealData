...
\begin{enumerate}
    ...
    \item Highlight pixels with brightness larger than the $n\%$ percentile.
    \item But any blobs with less than $m$ pixels highlighted would be considered too small to keep, and be discarded.
\end{enumerate}
Via manual fitting, $n=95\%, m=40$ was found to be the best algorithms.

\subsection{Extracting the outline}\label{sec:outlineDetermination}
Once the background is determined, it is fairly easy to determine what is in the foreground using simple negation of boolean variable. But determining the outline for each blob the pixel is a bit more difficult. After some experimentation, I have chosen to do it the lazy but fast way, which is to ``keep walking clockwise while touching the interface between the background-foreground element until we return to the starting point''.
...
\end{figure}
The Frangi (Figure~\ref{fig:u-frangi-4-18-2}), Meijerling (Figure~\ref{fig:u-meijerling-4-18-2}), and Butterworth (Figure~\ref{fig:u-butterworth}) filter managed to highlight the track sufficiently, so that when a simple global-threshold algorithm or the hysteresis-threshold algorithm is applied, it can extract the outline of the tracks left behind by the two particles.
\begin{figure}
\centering
...
\begin{figure}
\centering
\includegraphics[width=0.5\textwidth]{latexDoc/u-meijerling-4-18-2.png}
\caption{Meijerling filter applied\label{fig:u-meijerling-4-18-2}}
\end{figure}
\begin{figure}
\centering
